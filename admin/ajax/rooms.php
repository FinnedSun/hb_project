...
if (isset($_POST['add_room'])) {
    $features = filteration(json_encode($_POST['features']));
    $facilities = filteration(json_encode($_POST['facilities']));
    ...
    if ($stmt = mysqli_prepare($con, $q2)) {
        ...
        $flag = 0;
        die('query cannot be preppared - insert');
    }
...
if (isset($_POST['get_all_rooms'])) {
    $res = select("SELECT * FROM `rooms` WHERE `removed`=? ORDER BY `id` DESC", [0], 'i');
    $i = 1;
        ...
        $data .= "
                <tr>
                    ...
                    <td class='none'>
                        ...
                        <button type='button' onclick=\"room_images($row[id],'$row[name]')\" class='btn btn-info btn-sm shadow' data-bs-toggle='modal' data-bs-target='#room-images'>
                            <i class='bi bi-images'></i>
                        </button>
                        <button type='button' onclick='remove_room($row[id])' class='btn btn-danger btn-sm shadow'>
...
if (isset($_POST['edit_room'])) {
    ...
    $del_features = insert("DELETE FROM `room_features` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
    $del_facilities = insert("DELETE FROM `room_facilities` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
...
if (isset($_POST["get_room_images"])) {
    $frm_data = filteration($_POST);
    $res = select("SELECT * FROM `room_images` WHERE `room_id`=? ORDER BY `sr_no` DESC", [$frm_data['get_room_images']], 'i');
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        if ($row['thumb'] == 1) {
            ...
            $thumb_btn = "<button onclick='thumb_image($row[sr_no],$row[room_id])' class='btn btn-secondary'><i class='bi bi-check-lg'></i></button>";
        }
        echo <<<data
...
if (isset($_POST["remove_room"])) {
    ...
    $res3 = update("DELETE FROM `room_features` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
    $res4 = update("DELETE FROM `room_facilities` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
